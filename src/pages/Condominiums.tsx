            ...
            {pageItems.map(c => {
              ...
              return (
                ...
                >
                  ...
                  {(() => {
                    const implant = c.implantacao_url?.trim() || c.mapa_pdf_url?.trim() || null;
                    const drive = driveOf(c);
                    if (!hasFotos && !implant && !drive) return null;
                    const cls = "inline-flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground hover:text-primary transition-colors disabled:opacity-60";
                    return (
                      <div className="flex items-center gap-4 px-4 py-2 border-t border-border/60">
                        {hasFotos && (
                          <button
                            title="Baixar fotos"
                          ...
                          >
                            {zipping === c.id
                              ...
                              : <Download className="w-3.5 h-3.5" />} Fotos
                          </button>
                        )}
                        {implant && (
                          <button
                            title="Ver implantação"
                            onClick={e => { e.stopPropagation(); window.open(implant, "_blank", "noopener,noreferrer"); }}
                            className={cls}
                          >
                            <MapIcon className="w-3.5 h-3.5" /> Implant.
                          </button>
                        )}
                        {drive && (
                          <button
                            title="Acessar Drive"
                            onClick={e => { e.stopPropagation(); window.open(drive, "_blank", "noopener,noreferrer"); }}
                            className={cls}
                          >
